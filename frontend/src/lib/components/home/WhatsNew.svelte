<script lang="ts">
	import type { WhatsNewSectionComponent, StrapiTrail } from "$lib/types/strapi";
	...
	<div class="mx-auto max-w-7xl">
		{#if section.title}
			<header class="mb-16 flex items-center justify-between">
				<h2 class="text-sm font-medium tracking-[0.3em] uppercase text-stone-400">
					{section.title}
				</h2>
		...
		<div class="grid gap-12 sm:gap-x-10 sm:gap-y-16">
			{#each trails as trail, i}
				...
				<div
					...
					{layout === 'asymmetric' ? (i % 2 === 0 ? 'col-span-12 sm:col-span-5' : 'col-span-12 sm:col-span-7') : ''}"
				>
					<div class="relative aspect-[4/5] overflow-hidden rounded-sm bg-stone-100 sm:aspect-[3/2]">
						<img
						...
						<div class="absolute inset-0 bg-black/5 opacity-0 transition-opacity duration-500 group-hover:opacity-100 pointer-events-none"></div>
					</div>

					<div class="mt-6 flex items-start justify-between gap-6">
						<!-- Left side: Title and Summary -->
						<div class="flex-1">
							<h3 class="text-xl font-light text-stone-900 tracking-tight sm:text-2xl group-hover:text-stone-600 transition-colors duration-300">
								{trail.title}
							</h3>
							{#if trail.summary}
								<p class="mt-2 text-sm text-stone-500 leading-relaxed max-w-md line-clamp-2">
									{trail.summary}
								</p>
							{/if}
						</div>
						...
						<div class="flex flex-col items-end pt-1">
							<div class="flex flex-col items-end gap-1">
								{#if trail.distance_km}
									<span class="text-[0.65rem] font-medium tracking-[0.2em] uppercase text-stone-400">
										{trail.distance_km} KM
									</span>
								{/if}
								{#if trail.difficulty}
									<span class="text-[0.65rem] font-medium tracking-[0.2em] uppercase text-stone-800">
										{trail.difficulty}
									</span>
								{/if}
							</div>
